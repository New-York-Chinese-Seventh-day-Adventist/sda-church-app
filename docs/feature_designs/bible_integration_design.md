...
- **No Auth:** Open access to public domain and Creative Commons translations (BSB, WEB, KJV, CUV) requires no API keys.
...
- **Rate Limiting:** The service does not enforce restrictive per-user rate limits and utilizes a global CDN for high availability. As it focuses on public domain and open-licensed translations, there are no commercial usage tiers. The underlying infrastructure (AWS S3/CloudFront) provides high scalability and cost-efficiency.

https://bible.helloao.org/docs/reference/#available-translations

#### 2.2 Longevity

1. **Legal Sustainability (Non-Profit Governance)**

AO Lab is a 501(c)(3) non-profit organization dedicated to open-access Bible data. Unlike commercial startups, its governance model is mission-driven rather than profit-driven.
...
2. **Technical Resilience (Open Architecture)**
...
https://github.com/HelloAOLab/bible-api forked to https://github.com/CodeSammich/bible-api
...
2.  **Branding Constraints:** It is nearly impossible to inject custom CSS into an iframe to match the app's `react-native-paper` Material 3 theme and `primary` color (`#0061A4`).
...
4.  **UI Overhead:** Rerendering a WebView is significantly heavier for the device than rendering a standard `FlashList` of text strings.
...
1. **Request Quotas:** The free tier is limited to 5,000 API calls per month, which may be insufficient for a growing community.
...
While the NIV is a global standard, it introduces several "Enterprise" hurdles that conflict with a smooth native PWA experience:

1. **Legal Risk:** Biblica owns the copyright. Usage is "low risk but not zero risk," requiring strict adherence to [Biblica Permissions](https://www.biblica.com/permissions/). High-volume usage often requires commercial licensing.
...
3. **Caching Restrictions:** Most commercial licenses forbid local storage/caching of the full text. This would break our goal of using `IndexedDB` for a 100% offline-capable experience.
